...
import archiveDDL from "src/app/CargarDDLs/ProduccionDDL";
  ...
  const guardarEmpleado = async (data) => {
    ...
    if (errors == 0 && formatos == 0) {
      ...
      const payload = {
        ...
        empl_FechaCreacion: formattedCurrentDate,
      };

      const response = await apiRequests.insertEmpleados(payload);
                ...
                <FormControl error={!!errors.empl_Sexo} fullWidth={true}>
                  ...
                  >
                    <FormControlLabel
                      ...
                      onChange={(e) => {
                        field.onChange(e);
                        setSexo(e.target.value); // Actualiza el estado de sexo
                      }}
                    />
                    <FormControlLabel
                      value="M"
