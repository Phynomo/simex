...
//Import service
import TipoLiquidacionServices from "./TipoLiquidacionService";
//Import ddls
...
function TipoLiquidacionIndex(){
  ...
  const TipoLiquidacionGetData = async () => {
    try {
      setData(await TipoLiquidacionServices.listar());
    } catch (error) {
    }
  };
  ...
  const TipoLiquidacionCreate = async () => {
    try {
      const response = await TipoLiquidacionServices.crear(datosWatch);
  ...
  const TipoLiquidacionEdit = async () => {
    try {
      const response = await TipoLiquidacionServices.editar(datosWatch);
      if (response.data.data.messageStatus == "1") {
  ...
  return (
    <Card sx={{ minWidth: 275, margin: "40px" }}>
      <CardMedia
        component="img"
        height="200"
        image=" https://i.ibb.co/GFY274D/TIPO-DE-LIQUIDACI-N.png"
