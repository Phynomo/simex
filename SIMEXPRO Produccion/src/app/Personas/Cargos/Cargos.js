...
const accountSchema = yup.object().shape({
  id: yup.string(),
  carg_Nombre: yup.string().trim().required(""),
});
...
function CargosIndex() {
  ...
  const handleEdit = (datos) => {
    ...
    setValue("id", datos["carg_Id"]);
    setValue("carg_Nombre", datos["carg_Nombre"]);
    handleClose(datos.carg_Id);
  };
  ...
  const columns = [
...

    {
      ...
      key: "carg_Nombre",
      sorter: (a, b) => a.carg_Nombre.localeCompare(b.carg_Nombre), //sorting para Letras
    },
    {
      title: "Acciones",
      key: "operation",
      render: (params) => (
        <div key={params.carg_Id}>
          <Stack direction="row" spacing={1}>
            <Button
              aria-controls={`menu-${params.carg_Id}`}
              aria-haspopup="true"
              onClick={(e) => handleClick(e, params.carg_Id)}
              variant="contained"
              style={{
                borderRadius: "10px",
            ...
            <Menu
              id={`menu-${params.carg_Id}`}
              anchorEl={anchorEl[params.carg_Id]}
              keepMounted
              open={Boolean(anchorEl[params.carg_Id])}
              onClose={() => handleClose(params.carg_Id)}
            >
  ...
  const camposToFilter = ["key", "carg_Nombre"];
  ...
  const { handleSubmit, register, reset, control, watch, formState, setValue } =
    useForm({
      defaultCargosValues, //Campos del formulario
      mode: "all",
  ...
  const CargosGetData = async () => {
    try {
      setData(await cargosService.listar());
    } catch (error) {
  ...
  const CargosCreate = async () => {
    try {
      const response = await cargosService.crear(datosWatch);
  ...
  const cargosEdit = async () => {
    try {
      const response = await cargosService.editar(datosWatch);
      if (response.data.data.messageStatus == "1") {
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <Divider style={{ marginTop: "0px", marginBottom: "0px" }}>
                  <Chip
                    label={editar ? "Editar cargo" : "Agregar cargo"}
                  />
                </Divider>
              </Grid>
              <Grid item xs={2}>
              ...
              <Grid item xs={8}>
                <FormControl fullWidth>
                  <FormLabel error={!!errors.carg_Nombre}>Nombre</FormLabel>
                  <Controller
                    render={({ field }) => (
                      <TextField
                        {...field}
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} style={{ marginBottom: "30px" }}>
              <Divider style={{ marginTop: "0px", marginBottom: "10px" }}>
                <Chip label="Detalles de el cargo" />
              </Divider>
            </Grid>
            ...
            >
              <Box sx={{ flex: 1, textAlign: "center" }}>
                <InputLabel htmlFor="id">
                  ...
                  <Typography>{DatosDetalles["carg_Id"]}</Typography>
                </InputLabel>
              </Box>
              <Box sx={{ flex: 1, textAlign: "center" }}>
                <InputLabel htmlFor="descripcion">
                  ...
                  <Typography>{DatosDetalles["carg_Nombre"]}</Typography>
                </InputLabel>
              </Box>
            </Grid>
              ...
              >
                ...
                <tbody>
                  <tr style={estilosTablaDetalles.tableRowStyle}>
                    ...
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["carg_FechaCreacion"]
                        ? new Date(
                          DatosDetalles["carg_FechaCreacion"]
                        ).toLocaleString()
                        : ""}
                  ...
                  <tr style={estilosTablaDetalles.tableRowStyle}>
                    ...
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["carg_FechaModificacion"]
                        ? new Date(
                          DatosDetalles["carg_FechaModificacion"]
                        ).toLocaleString()
                        : ""}
