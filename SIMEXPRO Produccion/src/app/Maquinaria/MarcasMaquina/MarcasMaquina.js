...
//Import service
import MarcasMaquinaServices from "./MarcasMaquinaService";
//Import ddls
...
const defaultMarcasMaquinasValues= {
  marq_Id: "",
  marq_Nombre: "",
}
...
const accountSchema = yup.object().shape({
  marq_Id: yup.string(),
  marq_Nombre: yup.string().trim().required(""),
});


function MarcasMaquinaIndex(){
  ...
  const VisibilidadTabla = () => {
    ...
    reset(defaultMarcasMaquinasValues);
  }; 
  ...
  const handleEdit = (datos) => {
    ...
    setValue("marq_Id", datos["marq_Id"]);
    setValue("marq_Nombre", datos["marq_Nombre"]);
    handleClose(datos.marq_Id);
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "operation",
      render: (params) => (
        <div key={params.marq_Id}>
          <Stack direction="row" spacing={1}>
            <Button
              aria-controls={`menu-${params.marq_Id}`}
              aria-haspopup="true"
              onClick={(e) => handleClick(e, params.marq_Id)}
              variant="contained"
              style={{
                borderRadius: "10px",
            ...
            <Menu
              id={`menu-${params.marq_Id}`}
              anchorEl={anchorEl[params.marq_Id]}
              keepMounted
              open={Boolean(anchorEl[params.marq_Id])}
              onClose={() => handleClose(params.marq_Id)}
            >
  ...
  const camposToFilter = ["key", "marq_Nombre"];
  ...
  const { handleSubmit, register, reset, control, watch, formState, setValue } =
    useForm({
      defaultMarcasMaquinasValues, //Campos del formulario
      mode: "all",
  ...
  const MarcasMaquinaGetData = async () => {
    try {
      setData(await MarcasMaquinaServices.listar());
    } catch (error) {
    }
  };

  //Peticion para crear un registro
  const MarcasMaquinaCreate = async () => {
    try {
      const response = await MarcasMaquinaServices.crear(datosWatch);
      if (response.data.data.messageStatus == "1") {
        ToastSuccess("El registro se ha insertado exitosamente");
        MarcasMaquinaGetData();
        VisibilidadTabla();
        reset(defaultMarcasMaquinasValues);
  ...
  const MarcasMaquinaEdit = async () => {
    try {
      const response = await MarcasMaquinaServices.editar(datosWatch);
      if (response.data.data.messageStatus == "1") {
  ...
  const MarcasMaquinaDelete = async () => {
    try{
      const response = await MarcasMaquinaServices.eliminar(datosWatch);
      if(response.data.data.messageStatus == "1"){
        DialogEliminar();
            ...
            <Grid container spacing={3}>
              <Grid item xs={12}>
                <Divider style={{ marginTop: "0px", marginBottom: "0px" }}>
                  <Chip
                    label={editar ? "Editar Marca de Maquina" : "Agregar Marca de Maquina"}
                  />
                </Divider>
              </Grid>
              <Grid item xs={2}>
              ...
              <Grid item xs={8}>
                <FormControl fullWidth>
                  <FormLabel error={!!errors.marq_Nombre}>Marca</FormLabel>
                  <Controller
                    render={({ field }) => (
                      <TextField
                        {...field}
            ...
            >
              <Box sx={{ flex: 1, textAlign: "center" }}>
                <InputLabel htmlFor="id">
                  ...
                  <Typography>{DatosDetalles["marq_Id"]}</Typography>
                </InputLabel>
              </Box>
              <Box sx={{ flex: 1, textAlign: "center" }}>
                <InputLabel htmlFor="descripcion">
                  ...
                  <Typography>{DatosDetalles["marq_Nombre"]}</Typography>
                </InputLabel>
              </Box>
            </Grid>
              ...
              >
                ...
                <tbody>
                  <tr style={estilosTablaDetalles.tableRowStyle}>
                    ...
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["marq_FechaCreacion"]
                        ? new Date(
                            DatosDetalles["marq_FechaCreacion"]
                          ).toLocaleString()
                        : ""}
                  ...
                  <tr style={estilosTablaDetalles.tableRowStyle}>
                    ...
                    <td style={estilosTablaDetalles.tableCellStyle}>
                      {DatosDetalles["marq_FechaModificacion"]
                        ? new Date(
                            DatosDetalles["marq_FechaModificacion"]
                          ).toLocaleString()
                        : ""}
...
export default MarcasMaquinaIndex;